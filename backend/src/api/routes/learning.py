...
from app.models.db import DocumentDomainMap, KnowledgeDomain, MicroModule, QuizAttempt, UserProgress
...
async def get_review_queue(
    ...
    def _base_filters():
        ...
        if domain_id is not None:
            # 需要 join DocumentDomainMap，用 subquery 取得符合領域的 doc_id 集合
...
async def retry_modules(
    ...
    """將指定條件的 next_review 重設為現在，讓這些模組立即出現在佇列中。"""
    # 取得符合條件的模組 ID
    ...
    # 將已有的 UserProgress next_review 重設為現在
    now = datetime.utcnow()
    progress_result = await db.execute(
        select(UserProgress).where(
            (UserProgress.user_id == user_id)
    ...
    # 重新用 get_review_queue 邏輯取得佇列（直接查）
