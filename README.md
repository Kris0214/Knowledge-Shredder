...
| 資料驗證 | Pydantic 2.9 |
| LLM | 多 Provider 可插拔設計（`openai` SDK 1.51） |
...
實作 SuperMemo SM-2 演算法，核心思想為：**答得越好，下次複習的間隔就拉得越長**，讓學習者在「快要忘記之前」才複習，達到最高記憶效率。

#### 三個核心變數

| 變數 | 初始值 | 意義 |
|------|--------|------|
| `repetitions` | 0 | 累計連續答對次數 |
| `interval_days` | 1 | 幾天後再複習 |
| `ease_factor`（EF） | 2.5 | 此模組對學習者的容易程度（下限 1.3） |

#### 每次作答的計算流程

**Step 1：將作答結果轉為 q 值（0–5 量表）**

本專案採二元評分，答對或答錯對應固定分數：

| 作答結果 | score | q = round(score × 5) |
|----------|-------|----------------------|
| 答對 | 1.0 | **5** |
| 答錯 | 0.0 | **0** |
...
| 條件 | 新間隔 | 說明 |
|------|--------|------|
| q < 3（答錯） | **1 天**，`repetitions` 歸零 | 明天立刻重考 |
| 第 1 次答對（repetitions=0） | **1 天** | 剛學，明天先鞏固 |
| 第 2 次答對（repetitions=1） | **6 天** | SM-2 原版實驗出的最佳短期間隔 |
| 第 3 次以後答對 | `上次間隔 × EF` | 根據表現動態拉長間隔 |

**Step 3：更新難易係數（EF）**

$$EF_{new} = EF + \bigl(0.1 - (5-q) \times (0.08 + (5-q) \times 0.02)\bigr)$$

由於本專案 q 只有 0 或 5，實際效果如下：

| 作答 | q | EF 變化量 | 說明 |
|------|---|-----------|------|
| 答對 | 5 | **+0.1** | 間隔倍數增加，下次等更久 |
| 答錯 | 0 | **−0.8** | 間隔倍數縮小，之後複習更頻繁 |

#### 具體範例

假設學習者學習「什麼是 ETF？」這個模組並連續答對：

| 次數 | 作答 | `repetitions` | `interval_days` | `ease_factor` | 下次複習 |
|------|------|---------------|-----------------|---------------|----------|
| 初始 | — | 0 | 1 | 2.5 | — |
| 第 1 次 | 答對 | 1 | 1 天 | 2.6 | 明天 |
| 第 2 次 | 答對 | 2 | 6 天 | 2.7 | 6 天後 |
| 第 3 次 | 答對 | 3 | 6 × 2.7 = **16 天** | 2.8 | 16 天後 |
| 第 4 次 | 答對 | 4 | 16 × 2.8 = **45 天** | 2.9 | 45 天後 |

若某次**答錯**：

```
答錯 → repetitions 歸零、interval 重置為 1 天、EF 降低 0.8
     → 明天立刻再考一次，且因 EF 降低，之後的間隔也會比答錯前短
```

複習時間軸示意：

```
4/24  4/25  4/30        5/16                        6/30
 │     │     │           │                           │
[複習] [複習] [複習]      [複習]         ...          [複習]
 +1天   +6天  +16天       +45天（越來越少打擾你）
```
